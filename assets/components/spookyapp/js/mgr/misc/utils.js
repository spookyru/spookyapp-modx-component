SpookyApp.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

SpookyApp.utils.getMenu = function (actions, grid, selected) {
    const menu = [];
...
SpookyApp.utils.renderActions = function (value, props, row) {
    const res = [];
    ...
    for (const i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="spookyapp-btn spookyapp-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="spookyapp-row-actions">{0}</ul>',
        res.join('')
    );
};